...
public class DeleteSuppliesServlet extends HttpServlet
{
    ...
    throws ServletException, IOException
  {
    ...
    new AdminService().log(user_name, name1, "耗材记录删除", new getRemortIP().getRemoteAddress(request), (String)request.getSession().getAttribute("sys"));

    String ids = request.getParameter("ids");
    System.out.println("supplies id：" + ids);
    String[] id = ids.split(",");
    int count = 0;
    for (int i = 0; i < id.length; ++i) {
      int rs = new AdminService().delteSupplies(id[i]);
      ++count;
    }
    out.write(count);
    out.close();
