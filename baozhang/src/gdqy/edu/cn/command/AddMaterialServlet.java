...
public class AddMaterialServlet extends HttpServlet
{
    ...
    throws ServletException, IOException
  {
    ...
    new AdminService().log(user_name, name1, "添加耗材", new getRemortIP().getRemoteAddress(request), (String)request.getSession().getAttribute("sys"));
    ...
    System.out.println(name + "-" + "-" + provider + "-" + standard + "-" + category_id + "-" + category_name + "-" + remark);

    boolean rs = false;
    int rs1 = 0;

    rs1 = new AdminService().addMaterial(name, provider, standard, unit_price, unit, category_id, remark);
