...
public class AddSuppliesServlet extends HttpServlet
{
    ...
    throws ServletException, IOException
  {
    ...
    new AdminService().log(user_name, name1, "耗材记录添加", new getRemortIP().getRemoteAddress(request), (String)request.getSession().getAttribute("sys"));

    String bill_id = request.getParameter("bill_id");
    String number = request.getParameter("number");
    String material_id = request.getParameter("material_id");

    System.out.println(material_id + "-" + "-" + number + "-" + bill_id);

    boolean rs = false;
    int rs1 = 0;

    rs1 = new AdminService().addSupplies(material_id, number, bill_id);
    if (rs1 == 1)
      out.write("succ");
    out.close();
  }
