...
public class RePaidanServlet extends HttpServlet
{
  public void destroy()
  {
    super.destroy();
    ...
    throws ServletException, IOException
  {
    ...
    new AdminService().log(user_name, name1, "派单受理", new getRemortIP().getRemoteAddress(request), (String)request.getSession().getAttribute("sys"));

    String ids = request.getParameter("ids");
    String weixiu_id = request.getParameter("weixiu_id");
    String sys = request.getParameter("sys");
    //System.out.println("派单受理：" + sys);

    String[] id = ids.split(",");
    int count = 0;
    for (int i = 0; i < id.length; ++i) {
      int rs = new AdminService().RePaidan(id[i], weixiu_id);
      ++count;
    }
    out.write(count);
