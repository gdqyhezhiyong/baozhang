...
public class DeleteCategoryServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String name1=(String)request.getSession().getAttribute("name");
		 (new AdminService()).log(user_name,name1,"故障类型删除",(new getRemortIP()).getRemoteAddress(request),(String)request.getSession().getAttribute("sys"));
		
		 String id= request.getParameter("id");
		 String level= request.getParameter("level");
       // System.out.println("id��"+id);
        //System.out.println("c_level��"+level);
       
       
       int rs = (new AdminService()).delteCategory(id,level);
      if(rs==1)
			out.write("succ");
		out.close();
	}
