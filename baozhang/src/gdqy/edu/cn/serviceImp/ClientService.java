...
public class ClientService implements UserServices {
...
//获取故障类别
  ...
  public String login(String username, String password, String sys)
  {
    ...
    try
    {
      rs = DBHelper.getConnection().createStatement(1004, 1007).executeQuery(sql);
      int i = 0;
      while (rs.next()) {
        ++i;
      }
    ...
    label153: DBHelper.close();
    return flag;
  }

  
	//报修登记-网络故障
  public int saveBill(String bz_id, String name, String phone, String project2, String description, String order_date, 
                      String order_time, String address, String building_id)
  {
    ...
    try
    {
      ...
      sql = "insert into  t_bill(bz_id,bz_name,bz_phone,project_name,descriptions,order_date,order_time,address,building_id,status) values('" + 
             bz_id + "','" + name + "','" + phone + "','" + project2 + "','" + description + "','" + order_date + "','" + 
             order_time + "','" + address + "','" + building_id + "', 0)";
...
//用户信息更新
  public boolean updateUser(String bz_id, String name, String phone, String area1, String area2, 
		                    ...
		                    String area2_id){
    boolean rs = false;
    String sql = "update t_user set name='" + name + "',phon_number='" + phone + "', area1_id=" + area1_id + ",area1='" + 
                  area1 + "',area2='" + area2 + "', area2_id=" + area2_id + ",building_id=" + building_id + ",floor='" + floor + "',room='" +
                  room + "',building='" + building + "',isonce=1 where user_name='" + bz_id + "'";
    System.out.println(sql);
    try {
      DBHelper.getConnection().createStatement().execute(sql);
      rs = true;
                      ...
                      String area2_id, String sys, String weixiu_date)
  {
    ...
    int rs = 0;
    String sql = "";
    if ((project3_id == null) || ("".equals(project3_id))) project3_id = "0";
    try {
      if ("0".equals(sys))
      ...
      sql = "insert into  t_bill(bz_id,bz_name,bz_phone,descriptions,order_date,weixiu_date,order_time,building_id,project3_id,project1_name,project2_name,project3_name,area1_name,area2_name,building_name,floor,room,sys,status,jinji,satisfy) values('" + 
             bz_id + "','" + name + "','" + phone + "','" + description + "',getdate(),'" + weixiu_date + "','" +
             order_time + "','" + building_id + "','" + project3_id + "','" + project1 + "','" + project2 + "','" + project3 + "','" +
             area1 + "','" + area2 + "','" + building + "','" + floor + "','" + room + "'," + sys + ", 0,0,0)";

      DBHelper.getConnection().createStatement().execute(sql);
      
      
      //根据维修员负责的楼栋更新故障单维修员信息
      if ("0".equals(sys)){
        sql = "select u.name as name,u.phon_number as phone,u.user_name as user_name from t_user u inner join t_building b on u.user_name=b.weixiu_id where b.id=" + building_id;
      }
      if ("1".equals(sys)) {
      ...
      if (rsset.next()) {
        ...
        sql = "update t_bill set weixiu_name='" + weixiu_name + "', weixiu_id='" + weixiu_id + "',weixiu_phone='" + 
               weixiu_phone + "' where  id =( select max(id) from t_bill where bz_id='" + bz_id + "' and status=0)";
      ...
      //根据维修员负责故障的类型更新故障单维修员信息
      if (("0".equals(sys)) && ("南海".equals(area1))) {
        sql = "select weixiu_id,t_user.name as name,t_user.phon_number as phone from t_category inner join t_user on t_category.weixiu_id=t_user.user_name   where t_category.id='" + project3_id + "'";
        rsset = DBHelper.getConnection().createStatement().executeQuery(sql);
        if ((rsset.next()) && ("0".equals(sys))) {
          String weixiu_id = rsset.getString("weixiu_id");
          String weixiu_name = rsset.getString("name");
          String phone_number = rsset.getString("phone");
          sql = "update t_bill set weixiu_id='" + weixiu_id + "',weixiu_name='" + weixiu_name + "',weixiu_phone='" + phone_number + "' where  id =( select max(id) from t_bill where bz_id='" + bz_id + "' and status=0)";
          DBHelper.getConnection().createStatement().execute(sql);
        }
  ...
  //管理员
  public List<Bill> getAllBills(String sort, String dir, String sys, String area)
  {
    List l = new ArrayList();
  ...
  //维修员
  public List<Bill> getAllBills(String weixiu_id, String sort, String dir, String project1, String sys, String area)
  {
    List l = new ArrayList();
		              ...
		              String area1_id, String area2_id)
  {
    ...
    try
    {
      sql = "update t_bill set bz_phone='" + phone + "',area1_name='" + area1 + "',area2_name='" + 
        ...
        project2.trim() + "',project3_name='" + project3.trim() + "', descriptions='" + description.trim()+
        "', order_time='" + order_time + "' where id=" + id;
      DBHelper.getConnection().createStatement().execute(sql);
      rs = 1;
    } catch (SQLException e) {
      rs = 2;
